...
case $1 in
  ( 'luna-streaming' ) echo "product: $1";;
  (*) echo "pick luna-streaming" && exit;;
esac
...
case $1 in
  ...
  luna-streaming)
    echo "product is luna-streaming"
    echo "make antora.yml links"
    cd docs-src/luna-streaming-core
    rm antora.yml; ln -s antora-luna-streaming.yml antora.yml
    cd ../..
    echo "run the build"
    npm run build:local:luna-streaming
    ;;
