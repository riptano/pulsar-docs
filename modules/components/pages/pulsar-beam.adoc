...
The https://github.com/kafkaesque-io/pulsar-beam[Pulsar Beam^]{external-link-icon} project is an http based streaming and queueing system for use with Pulsar. You can send messages over HTTP, push messages to a webhook or cloud function, chain webhooks and functions together, or messages can be streamed via https://www.html5rocks.com/en/tutorials/eventsource/basics/[server-sent events^]{external-link-icon}.
