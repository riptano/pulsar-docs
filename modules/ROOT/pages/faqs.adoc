...
How is DataStax Apache Pulsar Connector (pulsar-sink) different from DataStax Apache Kafka Connector (kafka-sink?::

 Features provided by `pulsar-sink` that are not supported in `kafka-sink`:
...
Missing features in `pulsar-sink` compared with `kafka-sink`:

 * No support for `tinyint` (`int8bit`) and `smallint` (`int16bit`)

 * The key is always a String, but you can write JSON inside it
   ** The support is implemented in pulsar-sink, but not in Pulsar IO

 * The “value” of a “message property” is always a String
   ** You cannot map the message property to `__ttl` or to `__timestamp`
