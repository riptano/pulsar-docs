...
+
. Once the application is deployed and the Application status is green on the Installation Console's Application tab, open the Pulsar Admin Console in a browser. Use the hostname if you configured one when enabling TLS in Step 4. You can also find the hostname on the Installation Console's Config tab, under **Main Settings**.
