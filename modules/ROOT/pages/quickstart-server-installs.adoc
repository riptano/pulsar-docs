...
----
+
. Open the Luna Streaming Installation Console's URL in a browser. The URL was displayed after step 1. In the example command output above, it was http://10.101.34.161:8800. Remember to note the generated password, which you'll need soon. 
+
...
The Luna Streaming Installation Console presents its "Configure Luna Streaming" page. Example:
+
...
+
Scroll down the Configure Luna Streaming page. In the Values section, if you'll authenticate with username/password, copy the credentials for a subsequent Pulsar Admin Console login. Example:
+
image::luna-streaming-admin-console-sample-creds.png[Pulsar Admin Console Values fields as described in surrounding text]
+ 
...
For example, if you specified ip-10-101-34-161.srv101.dsinternal.org as the hostname in the Luna Streaming Installation Console's Config tab, and if you did not configure TLS, launch the Pulsar Admin Console in a browser:

http://ip-10-101-34-161.srv101.dsinternal.org 

Or open https://ip-10-101-34-161.srv101.dsinternal.org if you configured TLS from the Luna Streaming Installation Console. 

. Log in to the DataStax Luna Streaming Pulsar Admin Console. 
The username is `admin`. You can find the password in the Luna Streaming Installation Console's Config tab, under **Values**.
...
DataStax Luna Streaming is a Replicated KOTS application. You can update the application by using the procedures in the Version History tab of the Admin Console, or via the KOTS CLI. 
