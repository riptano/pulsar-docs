...
Launch the Luna Streaming Pulsar Admin Console in a browser by opening the URL that was specified in the Luna Streaming Installation Console. Log in with username `admin`. The generated and persisted password is also displayed on the Config tab of the Installation Console.
...
You can also access the Admin Console via the external load balancer endpoint in your cloud provider:
...
You can select your current tenant using the drop-down at the top right of the Topics page. v
