...
* A Google account with priveleges to install and configure Kubernetes resources. 
...
1. Leave `existingStorageClassName` blank and specify the storage class parameters below. +
+
[source,yaml]
----
  volumes:
...
2. Create a custom storage configuration as a `yaml` file (https://github.com/datastax/pulsar-helm-chart/blob/master/helm-chart-sources/pulsar/templates/bookkeeper/bookkeeper-storageclass.yaml[like this one]) and tell the Helm chart to use that storage configuration when it creates the BookKeeper PVCs. +
+
[source,yaml]
----
  volumes:
