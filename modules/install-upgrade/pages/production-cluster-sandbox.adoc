...
10 kb*   // average message payload size: 10 kilobytes
100 * // average message throughput: 100 messages/sec
(24 * 3600)    // TTL and retention period: 1 day
    = 259 MB
    ≅ .080 TB
----
.. We now know our cluster needs .080 TB of storage for Bookkeeper ledger data, so we can calculate the number of Bookkeeper nodes with the ledger disk capacity of 4TB and an 85% effective utilization ratio.
+
[source,plain]
----
Bookkeeper count(raw)=ceiling(.864/(4 * 0.85)) ≅ .25
----
...
[source,plain]
----
Broker count(raw)=ceiling(.25/5) ≅ 1
----
