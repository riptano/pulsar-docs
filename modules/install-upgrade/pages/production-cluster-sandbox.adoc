...
By default, Pulsar sets Bookkeeper https://pulsar.apache.org/docs/administration-zk-bk/#bookkeeper-persistence-policies[ack-quorum^]{external-link-icon} size to 2.
...
{activeTopics} *         // number of active topics
...
≅ 226 TB
----
We now know our cluster needs 226 TB of storage for Bookkeeper ledger data.

. Calculate the number of Bookkeeper nodes with an individual ledger disk capacity.
+
[source,plain]
----
Bookkeeper count(raw)=ceiling(226/(4 * 0.85)) = 66
----
...
[source,plain]
----
Broker count(raw)=ceiling(66/3) = 22
----
We will need 22 Brokers (one in each Zone) to serve messages. This should also be evenly divisible by the number of zones.
